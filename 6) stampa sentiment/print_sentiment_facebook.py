...
SENTIMENT_FILE = os.path.join(BASE_DIR, "sentiment_totali_facebook.xml")
...
# --- Leggi nomi.xml e ordina secondo facebook ---
...
for name in root_nomi.findall('name'):
    fb_id = name.get('facebook')
    artists_ord = list(range(1, 101))    
    if fb_id in artists:
        idx = artists.index(fb_id)
...
plt.title('Sentiment Facebook per artista')
plt.tight_layout()
plt.show()